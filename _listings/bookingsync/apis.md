---
name: BookingSync
x-slug: bookingsync
description: Vacation rental property management software. Save Time & Book More with
  our Reservation System, Channel Manager and Website Builder. Request a Demo!
image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/712-bookingsync.jpg
x-kinRank: "8"
x-alexaRank: "165601"
tags: API Provider
created: "2018-08-25"
modified: "2018-08-25"
url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-provider/master/_listings/bookingsync/apis.md
specificationVersion: "0.14"
apis:
- name: BookingSync
  x-api-slug: bookingsync
  description: Vacation rental property management software. Save Time & Book More
    with our Reservation System, Channel Manager and Website Builder. Request a Demo!
  image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/712-bookingsync.jpg
  humanURL: https://www.bookingsync.com/
  baseURL: https:///
  tags: API Provider
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-provider/master/_listings/bookingsync/openapi.md
x-common:
- type: x-base
  url: https://www.bookingsync.com/
- type: x-blog
  url: http://blog.bookingsync.com/
- type: x-blog-rss
  url: http://blog.bookingsync.com/en/news/feed
- type: x-crunchbase
  url: http://www.crunchbase.com/company/bookingsync
- type: x-crunchbase
  url: https://crunchbase.com/organization/bookingsync
- type: x-developer
  url: http://www.bookingsync.com/documentation/api
- type: x-email
  url: support@bookingsync.com
- type: x-github
  url: https://github.com/BookingSync
- type: x-twitter
  url: https://twitter.com/bookingsync
include: []
maintainers:
- FN: Kin Lane
  x-twitter: apievangelist
  email: info@apievangelist.com
---