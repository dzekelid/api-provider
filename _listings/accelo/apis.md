---
name: Accelo
x-slug: accelo
description: 'The #1 Service Operations Automation (ServOps) featuring sales, project
  management, email integration, retainer management, sales tracking, and support.'
image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/320-accelo.jpg
x-kinRank: "8"
x-alexaRank: "105620"
tags: API Provider
created: "2018-08-25"
modified: "2018-08-25"
url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-provider/master/_listings/accelo/apis.md
specificationVersion: "0.14"
apis:
- name: Accelo
  x-api-slug: accelo
  description: 'The #1 Service Operations Automation (ServOps) featuring sales, project
    management, email integration, retainer management, sales tracking, and support.'
  image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/320-accelo.jpg
  humanURL: https://www.accelo.com/
  baseURL: https:///
  tags: API Provider
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-provider/master/_listings/accelo/openapi.md
x-common:
- type: x-blog
  url: https://www.accelo.com/resources/blog/
- type: x-crunchbase
  url: https://crunchbase.com/organization/affinitylive
- type: x-developer
  url: https://api.accelo.com/docs/?_ga=2.33722019.342188080.1527277082-882498665.1527277082
- type: x-developer
  url: https://www.accelo.com/resources/apis/
- type: x-email
  url: support@accelo.com
- type: x-email
  url: sales@accelo.com
- type: x-github
  url: https://github.com/accelo
- type: x-twitter
  url: https://twitter.com/accelo
- type: x-website
  url: https://www.accelo.com/
include: []
maintainers:
- FN: Kin Lane
  x-twitter: apievangelist
  email: info@apievangelist.com
---