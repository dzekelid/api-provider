---
name: Campaign Monitor
x-slug: campaign-monitor
description: Campaign Monitor makes it easy for you to create, send, and optimize
  your email marketing campaigns.
image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/383-campaign-monitor.jpg
x-kinRank: "8"
x-alexaRank: "24393"
tags: API Provider
created: "2018-08-25"
modified: "2018-08-25"
url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-provider/master/_listings/campaign-monitor/apis.md
specificationVersion: "0.14"
apis:
- name: Campaign Monitor
  x-api-slug: campaign-monitor
  description: Campaign Monitor makes it easy for you to create, send, and optimize
    your email marketing campaigns.
  image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/383-campaign-monitor.jpg
  humanURL: http://www.campaignmonitor.com/
  baseURL: https:///
  tags: API Provider
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-provider/master/_listings/campaign-monitor/openapi.md
x-common:
- type: x-base
  url: https://api.createsend.com/
- type: x-blog
  url: http://www.campaignmonitor.com/blog/
- type: x-blog-rss
  url: http://campaignmonitor.com/feeds/blog
- type: x-crunchbase
  url: https://crunchbase.com/organization/campaign-monitor
- type: x-crunchbase
  url: http://www.crunchbase.com/company/campaign-monitor
- type: x-developer
  url: https://www.campaignmonitor.com/api/
- type: x-email
  url: info@campaignmonitor.com
- type: x-github
  url: https://github.com/campaignmonitor
- type: x-twitter
  url: https://twitter.com/campaignmonitor
- type: x-website
  url: http://www.campaignmonitor.com/
include: []
maintainers:
- FN: Kin Lane
  x-twitter: apievangelist
  email: info@apievangelist.com
---